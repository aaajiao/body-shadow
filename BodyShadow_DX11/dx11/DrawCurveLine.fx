...
void GS(point vs2ps input[1], inout TriangleStream<vs2ps> SpriteStream)
{
	...
	if(queueLength > 0 && queueLength != 1)
	{
		...
		for(int i=0; i<queueLength; i++)
	    {
	    	...
	    	if(i == 0)
	    	{
	    		p1 = mul(float4(pData[index].pos, 1), tVP);
	    		p2 = mul(float4(pData[index+amount].pos, 1), tVP);
	    	}
	    	else
	    	{
	    		p1 = mul(float4(pData[index-amount].pos, 1), tVP);
	    		p2 = mul(float4(pData[index].pos, 1), tVP);
	    	}
	    	p1 /= p1.w;
	    	p2 /= p2.w;
	    	tangent = p2 - p1;
	    	normal = normalize(float2(tangent.y, -tangent.x));
	    	off = normal * w * mul(float4(1, 1, 0, 0), tP).xy;
	
	        output.PosWVP = p1 + float4(off,0,0) * p1.w;
	        output.TexCd.xy = float2((float)i / (float)(queueLength-1), 0);	
	        output.TexCd.zw = tex;
	    	SpriteStream.Append(output);
	 
	        output.PosWVP = p2-float4(off,0,0)*p2.w;
